...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, lambdas: Tensor, attn_args: AttnArgs):
        ...
        if self.attn is not None:
            x = x + self.attn(norm(lambdas[2] * x + lambdas[3] * x0), attn_args)
        if self.mlp is not None:
            x = x + self.mlp(norm(lambdas[4] * x + lambdas[5] * x0))
        return x
...
class GPT(nn.Module):
        ...
        self.scalars = nn.Parameter(
            torch.cat(
                [
                    ...
                    *[
                        torch.tensor([1.0, 0.0, 1.0, 0.0, 1.0, 0.0]) for _ in range(num_layers)
                    ],  # block lambdas
                    *[
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, seqlens: Tensor, ws_short: int, ws_long: int):
        ...
        smear_lambda = self.scalars[9 * len(self.blocks)]
        ...
        backout_lambda = self.scalars[9 * len(self.blocks)+1]
